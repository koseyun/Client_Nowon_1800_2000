...
int main(void) {
	...
	// % 연산자
	intResult = 3 % 4;
	doubleResult = 3 % 4; // 정수형간의 연산 후에 암묵적으로 실수형으로 자료형 변환이 일어난 경우 (int)3.0 % (int)4.0
	printf("%d\n", intResult);
	printf("%lf\n", doubleResult);
	...
	intResult = 0;
	printf("intResult : %d\n", intResult);
	printf("intResult : %d\n", intResult--); // 후위 감소 연산자 , 해당 연산자가 포함된 줄을 실행한 후에 연산한다.
	printf("intResult : %d\n", intResult);
	intResult--;
	printf("intResult : %d\n", intResult);
	...
	// ==
	printf("is %d same with than %d ? %d\n", 3, 4, 3 == 4);
	...
	*/
	...
	// ^ ( 비트 xor 연산자. 피연산자 2개.)
	// 배교할 두 비트가 하나만 1일때 해당비트 1, 나머지경우 0
	printf("%d ^ %d = %d\n", 8, 42, 8 ^ 42);
