...
// 크기를 미리 전해줄 수 없고, 조건에 따라 맞는 크기를 확보해야 하는 경우가 있으니까
// 
// 동적으로 메모리를 할당한 후에 0으로 초기화 해주는 함수 :
// calloc(할당할 메모리 단위 수, 할당할 메모리 단위 크기)
// 
// 쓰는 이유 :
// 할당한 후에 쓰레기값이 들어 있으면 안되는 경우
// 
...
int main(int argc, char **argv) {
	...
	// 함수 호출스택이 해제될때, 해당 변수의 주소의 메모리는 같이 해제되기때문에
	// free 함수를 통해서 해당 메모리영역을 해제해줄 필요 없지만,
	// 동적 할당된 힙 영역의 메모리는 함수가 스택에서 해제되어도
	// 힙 영역의 메모리 영역이 해제되지 않기때문에 힙영역에 할당한 메모리의 주소를
