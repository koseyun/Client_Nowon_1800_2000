/*
    이 예시에서는 '경로관리자' CPathMgr을 작성하자 (그리고 작동시켰다)

    실행파일을 기준으로 현재 디렉토리의 전체 경로를 얻고 그러한 전체 경로의 하위 디렉토리로 원하는 경로를 덧붙인다

    이러한 경로정보들은 경로관리자에 해시테이블로 저장되어 빠르게 접근하여 사용하기 쉬운 형태로 추상화된다
