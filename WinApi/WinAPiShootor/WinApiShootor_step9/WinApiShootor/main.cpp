...
/*
    // winAPIShootor step9 ani file per frame

    a. 애니메이션 : '프레임'들을 '일정시간 간격'으로 빠르게 '순차적으로 보여주어' 눈의 잔상효과를 이용하여 마치 움지기이는 그림처럼 보이게 하는 것
    b. 스프라이트 : 게임에서 사용되는 작은 조각 이미지
    c. 스프라이트 애니메니션 : 스프라이트를 프레임 단위의 데이터로 사용하여 애니메이션 하는 것

    이번 예시에서는 스프라이트 애니메이션의 구현에 대해서 살펴본다

    다음과 같은 가정을 전제한다
        1. 스프라이트 프레임은 파일 단위로 준비한다
        2. 스프라이트 프레임들은 모두 같은 크기(너비,높이)이다
        3. 스프라이트 파일 이름은 접미사 형태로 '_번호'를 붙인다

    CAniSeq : 스프라이트 프레임의 집합, 하나의 애니메이션 시퀀스

    CAnimator : 다수개의 애니메이션 시퀀스(CAniSeq), 애니메이션 시퀀스의 제어(임의의 애니메이션 시퀀스 플레이)

    //ryu_ani_fileperframe
*/
