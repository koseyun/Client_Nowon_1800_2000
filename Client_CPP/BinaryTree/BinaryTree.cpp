...
Node& BinaryTree::Find(int item)
{
    ...
    while (_tmp != nullptr)
    {
        // 찾고자 하는 값이 현재 노드보다 작을때
        if (_tmp->_data > item)
            _tmp = _tmp->_left;
        // 찾고자 하는 값이 현재 노드보다 클때
...
bool BinaryTree::Remove(int item)
{
    ...
    while (_tmp != nullptr)
    {
        ...
        // 삭제하고자 하는 노드에 왼쪽 자식만 있을 경우
            ...
            // 삭제하고자 하는 노드의 자식 노드와 삭제하고자 하는 노드의 부모노드를 연결
            if (_tmp2 != nullptr) {
                if (dir == 0)
