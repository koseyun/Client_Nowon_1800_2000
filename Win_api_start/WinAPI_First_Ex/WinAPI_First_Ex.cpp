...
//윈도우 응용프로그램의 코드가 왜 이렇게 구조가 만들어져 있는지 분해해 보는 것이다. 
...
int APIENTRY wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow)
{
    //이것은 윈도우 클래스를 만들고
    //필요한 데이터들을 등록하는 것이다.
    // ( 여기서 클래스는 사용자정의타입을 만드는 예약어를 얘기하는 것이 아니다
    // 여기서 클래스는 분류의 의미이다.
    // 즉, 윈도우 클래스란 윈도우응용프로그램의 정보 중 일부를 나타내는 용어이다 )
    //
    // 윈도우 클래스 라는 개념이 있는 이유는
    // 중복된 데이터들을 공유하여 관리하기 위한 개념이다.
    // 그러면, 운영체제 입장에서 보다 효율적으로 자원이 관리되기 때문이다.
    ...
    wcex.style = CS_HREDRAW | CS_VREDRAW;
    wcex.lpfnWndProc = nullptr;//WndProc;
    wcex.cbClsExtra = 0;        //윈도우 클래스 추가 메모리
    wcex.cbWndExtra = 0;        //윈도우 추가 메모리
    wcex.hInstance = hInstance; //해당 윈도우 응용프로그램 식별자
    //wcex.hIcon = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_WINAPITEST2022061000));
    wcex.hIcon = LoadIcon(hInstance, nullptr);//MAKEINTRESOURCE(IDI_WINAPITEST2022061000));
    wcex.hCursor = LoadCursor(nullptr, IDC_ARROW);
    wcex.hbrBackground = (HBRUSH)(COLOR_WINDOW + 1);
    ...
    wcex.lpszMenuName = nullptr;//MAKEINTRESOURCEW(IDC_WINAPITEST2022061000);
    //wcex.lpszClassName = szWindowClass;
    wcex.lpszClassName = L"win_ryu";//szWindowClass; 윈도우클래스 이름
    //wcex.hIconSm = LoadIcon(wcex.hInstance, MAKEINTRESOURCE(IDI_SMALL));
    wcex.hIconSm = LoadIcon(wcex.hInstance, nullptr);//MAKEINTRESOURCE(IDI_SMALL));


    RegisterClassExW(&wcex);    //해당 윈도우 클래스 를 운영체제에 등록한다
