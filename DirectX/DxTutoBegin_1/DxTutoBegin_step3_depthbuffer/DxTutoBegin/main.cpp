/*
    ...
    2. Depth/Stencil Buffer 적용
        DirectX는 Z Buffer기법을 사용한다
    

    올바른 랜더링 처리를 위해서는 앞쪽에 있는 물체가 뒤쪽에 있는 물체를 가릴 수 있는 처리가 필요하다
    이를 위한 처리 방법은 크게 두가지가 있다
        a) Z sort : 정렬해야하므로 정렬에 시간이 많이 든다. 면(삼각형->정점 3개)단위이다
        삼각형 개수에 비례해서 정렬시간이 소요된다
        b) Z buffer : 물체가지의 거리를 픽셀 단위로 깊이버퍼에 기억시켜두고 랜더링시 랜더링하려는 물체의 거리와
                      이미 기억되어 있는 거리를 비교하여 앞에 있다고 판단되면 랜더링한다

        깊이버퍼(DepthBuffer(Z Buffer)는 정규화되어있다 [0, 1]
        값이 작을수록 앞에 있는 물체이다

        픽셀단위이므로 소요시간은 상수시간이다 O(1)
*/
